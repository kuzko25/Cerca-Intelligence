...
def procesar_con_motor_fastapi(archivo, params):
    ...
    try:
        r = requests.post("https://cerca-motor.onrender.com/procesar", files=files, params=params, timeout=500)
        return r.json()
...
def consultar_individual_api(cuil):
    try:
        r = requests.get(f"https://cerca-motor.onrender.com/consulta/{cuil}", timeout=500)
...
if st.session_state.user is None:
    ...
    if user_data:
        ...
        menu = option_menu(None, ["Inicio", "Cartera Masiva", "Consulta Individual", "Pagos", "Mi Perfil"], 
            icons=['house', 'cloud-upload', 'search', 'credit-card', 'person-circle'], 
